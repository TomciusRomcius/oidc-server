...
public class ClientController(IClientService clientService) : ControllerBase
{
    ...
    {
        ClientAggregate? result = await clientService.GetClientAsync(clientId);
        return Ok(new DataResponse<ClientAggregate?>(result));
    }
    ...
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<IActionResult> UpdateClientAsync([FromBody] CreateClientRequest request)
    {
        var error = await clientService.UpdateClientAsync(request);
        return error != null
            ? ControllerUtils.ResultErrorToResponse(error)
            : NoContent();
    }
